...
#Region "Database Initilization"
  ...
  Public Function CheckDatabaseTable(ByVal strTableName As String) As Boolean
    ...
    Try
      ...
      '
      If Regex.IsMatch(strTableName, "tblAudioDevices") = True Then
        '
        ...
        Else
          ...
          Using dbcmd As DbCommand = DBConnectionMain.CreateCommand()
            ...
            sqlQueue.Enqueue("CREATE TABLE IF NOT EXISTS tblAudioDevices(" _
                            & "device_id INTEGER PRIMARY KEY," _
                            & "device_name varchar(255) NOT NULL," _
                            & "device_serial varchar(255) NOT NULL," _
                            ...
                            & "device_zones INTEGER NOT NULL," _
                            & "device_tuner_src INTEGER NOT NULL," _
                            & "device_media_mgr INTEGER NOT NULL" _
                            & ")")

            sqlQueue.Enqueue("CREATE UNIQUE INDEX IF NOT EXISTS idxNAME ON tblAudioDevices(device_name)")
            sqlQueue.Enqueue("CREATE UNIQUE INDEX IF NOT EXISTS idx ON tblAudioDevices(device_serial)")
            sqlQueue.Enqueue("CREATE UNIQUE INDEX IF NOT EXISTS idxADDR ON tblAudioDevices(device_addr)")
      ...
      ElseIf Regex.IsMatch(strTableName, "tblAudioZoneSources") = True Then
        '
        ' Retrieve schema information about tblTableName
        '
        Dim SchemaTable As DataTable = DBConnectionMain.GetSchema("Columns", New String() {Nothing, Nothing, strTableName})
        ...
        Else
          ...
          Using dbcmd As DbCommand = DBConnectionMain.CreateCommand()
            ...
            Dim sqlQueue As New Queue
            sqlQueue.Enqueue("CREATE TABLE IF NOT EXISTS tblAudioZoneSources(" _
                            & "source_id INTEGER PRIMARY KEY," _
                            & "device_id INTEGER NOT NULL," _
                            & "source_number INTEGER NOT NULL," _
                            & "source_name varchar(255) NOT NULL," _
                            & "source_name_keypad varchar(255) NOT NULL," _
                            & "source_enabled INTEGER NOT NULL DEFAULT 1" _
                            & ")")

            sqlQueue.Enqueue("CREATE UNIQUE INDEX IF NOT EXISTS idxINPUT ON tblAudioZoneSources(device_id, source_number)")
