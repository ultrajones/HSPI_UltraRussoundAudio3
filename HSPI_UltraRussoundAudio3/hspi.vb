  ...
  Public Sub SetIOMulti(colSend As System.Collections.Generic.List(Of HomeSeerAPI.CAPI.CAPIControl)) Implements HomeSeerAPI.IPlugInAPI.SetIOMulti
    ...
    For Each CC In colSend
      ...
      Try
        ...
        '
        Select Case dv_type
          Case "Russound AudioZone Controller"

            Select Case dv_value
              Case Is < 0

                Dim AudioDevice As hspi_audio_device = gAudioDevices.Find(Function(s) s.DeviceId = strUUID)
                If Not IsNothing(AudioDevice) Then
                  Select Case dv_value
                    Case -1
                      AudioDevice.Reconnect()
                    Case -2
          ...
          Case "Russound AudioZone Power"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Dim controller_id As String = Regex.Match(dv_addr, regexPattern).Groups("cc").ToString()
              Dim zone_id As String = Regex.Match(dv_addr, regexPattern).Groups("zz").ToString()
              Dim device_type As String = Regex.Match(dv_addr, regexPattern).Groups("type").ToString()

              Select Case dv_value
                Case 0, 1
                  AudioDevice.SetZoneStateEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select

            End If

          Case "Russound AudioZone Source"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case 0, 1, 2, 3, 4, 5
                  AudioDevice.SetZoneSourceEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Party Mode"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -1
                  AudioDevice.SetZonePartyModeToggleEx(controller_id, zone_id)
                Case 0, 1, 2
                  AudioDevice.SetZonePartyModeEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Do Not Distrub"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -1
                  AudioDevice.SetZoneDoNotDisturbToggleEx(controller_id, zone_id)
                Case 0, 1
                  AudioDevice.SetZoneDoNotDisturbEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Keypad Background Color"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -1
                  AudioDevice.SetZoneBackgroundColorToggleEx(controller_id, zone_id)
                Case 0, 1, 2
                  AudioDevice.SetZoneBackgroundColorEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Turn On Volume"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetZoneTurnOnVolumeDecreaseEx(controller_id, zone_id)
                Case -1
                  AudioDevice.SetZoneTurnOnVolumeIncreaseEx(controller_id, zone_id)
                Case 0 To 50
                  AudioDevice.SetZoneTurnOnVolumeEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Volume"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case 0 To 50
                  AudioDevice.SetZoneVolumeEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Bass"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetZoneBassDecreaseEx(controller_id, zone_id)
                Case -1
                  AudioDevice.SetZoneBassIncreaseEx(controller_id, zone_id)
                Case 0 To &H14
                  AudioDevice.SetZoneBassEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Treble"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetZoneTrebleDecreaseEx(controller_id, zone_id)
                Case -1
                  AudioDevice.SetZoneTrebleIncreaseEx(controller_id, zone_id)
                Case 0 To &H14
                  AudioDevice.SetZoneTrebleEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Balance"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetZoneBalanceRightEx(controller_id, zone_id)
                Case -1
                  AudioDevice.SetZoneBalanceLeftEx(controller_id, zone_id)
                Case 0 To &H14
                  AudioDevice.SetZoneBalanceEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Loudness"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -1
                  AudioDevice.SetZoneLoudnessToggleEx(controller_id, zone_id)
                Case 0, 1
                  AudioDevice.SetZoneLoudnessEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone Source Controls"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case &H00
                Case Else
                  AudioDevice.SendZoneSourceControlEventEx(controller_id, zone_id, dv_value)
              End Select

            End If

          Case "Russound AudioZone Keypad Controls"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case &H00
                Case &H64, &H67, &H68, &H69, &H6A, &H6B, &H6C, &H6D, &H6E, &H6F, &H70, &H73, &H7F, &H80
                  AudioDevice.SendZoneKeypadControlEventEx(controller_id, zone_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound AudioZone System OnState"

          Case "Russound AudioZone Shared Source"

          Case "Russound Tuner Radio Controls"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Dim device_id As String = Regex.Match(dv_addr, regexPattern).Groups("dd").ToString()
              Dim controller_id As String = Regex.Match(dv_addr, regexPattern).Groups("cc").ToString()
              Dim tuner_id As String = Regex.Match(dv_addr, regexPattern).Groups("tt").ToString()
              Dim device_type As String = Regex.Match(dv_addr, regexPattern).Groups("type").ToString()

              Select Case dv_value
                Case &H00
                Case &H0A - &H09, &H1B, &H46, &H49, &H4A, &H13, &H33, &H34, &H42, &H43
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, dv_value)
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound Tuner Power"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.POWER_OFF)
                Case -1
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.POWER_ON)
                Case 0
                Case Else
                  WriteMessage(String.Format("Received unsupported SetIOMulti action {0} for {1}", dv_type, dv_value), MessageType.Warning)
              End Select
          ...
          Case "Russound Tuner Radio Frequency", "Russound Tuner XM Channel"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.TUNE_DOWN)
                Case -1
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.TUNE_UP)
                Case 0
                Case Else
                  AudioDevice.TunerDirectTuneMode(controller_id, tuner_id, dv_value)
              End Select

            End If

          Case "Russound Tuner Bank"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.BANK_DOWN)
                Case -1
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.BANK_UP)
                Case 0
                Case Else
                  AudioDevice.TunerDirectBankSelect(controller_id, tuner_id, dv_value)
              End Select

            End If

          Case "Russound Tuner Preset"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.PRESET_DOWN)
                Case -1
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.PRESET_UP)
                Case 0
                Case Else
                  AudioDevice.TunerDirectPresetSelect(controller_id, tuner_id, dv_value)
              End Select

            End If

          Case "Russound Tuner XM Category"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.CATEGORY_DOWN)
                Case -1
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.CATEGORY_UP)
                Case 0
                Case Else
                  AudioDevice.TunerDirectCategorySelect(controller_id, tuner_id, dv_value)
              End Select

            End If

          Case "Russound Tuner XM Category Channel"
            ...
            If Not IsNothing(AudioDevice) Then
              ...
              Select Case dv_value
                Case -2
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.CAT_CHANNEL_DOWN)
                Case -1
                  AudioDevice.SetTunerCommandEx(controller_id, tuner_id, hspi_audio_device.TunerCommand.CAT_CHANNEL_UP)
              End Select
  ...
  Public Function InitIO(ByVal port As String) As String Implements HomeSeerAPI.IPlugInAPI.InitIO
    ...
    Try
      ...
      If bDBInitialized = True Then
        ...
        '
        CheckDatabaseTable("tblAudioDevices")
        CheckDatabaseTable("tblAudioZoneSources")
        ...
        '
        ConnectionThread = New Thread(New ThreadStart(AddressOf AudioDeviceConnection))
